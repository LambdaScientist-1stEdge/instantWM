...
static inline bool checkfloating(Client *c) {
    return (c->isfloating || NULL == selmon->lt[selmon->sellt]->arrange);
}
...
setoverlay() {
	if (!overlayexists()) {
	} else if (!selmon->overlaystatus) {
		showoverlay();
	} else if (ISVISIBLE(selmon->overlay)) {
		hideoverlay();
	} else {
		showoverlay();
	}
...
applyrules(Client *c)
{
	...
	class    = ch.res_class ?: broken;
	instance = ch.res_name  ?: broken;
	...
	if (specialnext) {
		...
		for (size_t i = 0; i < LENGTH(rules); ++i) {
			...
			&& (!r->instance || strstr(instance, r->instance))){
				...
				c->tags |= r->tags;
				Monitor *m = mons;
				while(m && m->num != r->monitor) m = m->next;
				if (m)
					c->mon = m;
			}
		}
...
cleanup(void)
{
	...
	for (size_t i = 0; i < LENGTH(colors) + 1; ++i)
		free(scheme[i]);
	...
	if (cme->message_type == netatom[NetWMState]) {
		...
		if (c == c->mon->overlay) {
			...
			if (HIDDEN(c))
				show(c);
			size_t i = 0;
			while (i < LENGTH(tags) && !((1 << i) & c->tags)) {++i;}
			if (i < LENGTH(tags)) {
				const Arg a = {.ui = 1 << i};
				if (selmon != c->mon) {
					unfocus(selmon->sel, 0);
...
cyclelayout(const Arg *arg) {
	...
	while(l != selmon->lt[selmon->sellt]){++l;}
	if(arg->i > 0) {
...
detach(Client *c)
{
	...
	while(*tc && *tc != c){tc = &(*tc)->next;};
	*tc = c->next;
}
void
detachstack(Client *c)
{
	Client **tc = &c->mon->stack;
	while (*tc && *tc != c){ tc = &(*tc)->snext;}
	*tc = c->snext;

	if (c == c->mon->sel) {
		Client *t = c->mon->stack;
		while(t && !ISVISIBLE(t)){t = t->snext;}
		c->mon->sel = t;
	}
}
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - x - stw) > bh) {
		if (n > 0) {
			...
			if (!selmon->clients) {
				int titlewidth =
						TEXTW("Press space to launch an application") < selmon->btw
								? TEXTW("Press space to launch an application")
								: (selmon->btw - bh);
				drw_text(drw, x + bh + ((selmon->btw - bh) - titlewidth + 1) / 2,
						0, titlewidth, bh, 0, "Press space to launch an application", 0, 0);
			}
		}
	}
	drw_setscheme(drw, scheme[SchemeNorm]);
...
enternotify(XEvent *e){
	...
	Client *c = wintoclient(ev->window);
	if (c && selmon->sel 
	&& ( selmon->sel->isfloating || NULL == selmon->lt[selmon->sellt]->arrange ) 
	&& c != selmon->sel 
...
focus(Client *c)
{
	...
	if (!c || !ISVISIBLE(c) || HIDDEN(c)){
		c = selmon->stack;
		while(c && (!ISVISIBLE(c) || HIDDEN(c))) {c = c->snext;}
	}
	...
	selmon->sel = c;
	if (selmon->gesture || selmon->gesture < 9)
		selmon->gesture = 0;
...
focusstack(const Arg *arg)
{
	...
	if (arg->i > 0) {
		...
		for (c = selmon->clients; c != selmon->sel && (!ISVISIBLE(c) || HIDDEN(c)) ; c = c->next);
	}
	if (c) {
		focus(c);
		restack(selmon);
...
grabkeys(void)
{
	...
	if(!selmon->sel){
		for (size_t i = 0; i < LENGTH(dkeys); i++) {
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if (animated && !c->isfullscreen) {
		...
		if (NULL == c->mon->lt[selmon->sellt]->arrange ) {
			XRaiseWindow(dpy, c->win);
		} else if (c->w > selmon->mw - 30 || c->h > selmon->mh - 30){
			arrange(selmon);
		}
	}
}
...
maprequest(XEvent *e)
{
	...
	Client *c = wintosystrayicon(ev->window);
	if (c) {
...
motionnotify(XEvent *e)
{
	...
	if (ev->x_root < selmon->mx + tagwidth + startmenusize) {
		...
		if (ev->x_root > selmon->activeoffset && ev->x_root < (selmon->activeoffset + 32)) {
			...
			if (!altcursor) {
				if (ev->x_root > selmon->activeoffset + (1.0 / (double)selmon->bt) * selmon->btw - 30
				&& ev->x_root < selmon->activeoffset + (1.0 / (double)selmon->bt) * selmon->btw) {
					XDefineCursor(dpy, root, cursor[CurResize]->cursor);
					altcursor = 1;
				}
			} else if (ev->x_root < selmon->activeoffset + (1.0 / (double)selmon->bt) * selmon->btw -	30 
						 || ev->x_root > selmon->activeoffset + (1.0 / (double)selmon->bt) * selmon->btw) {
				XDefineCursor(dpy, root, cursor[CurNormal]->cursor);
				altcursor = 0;
			}
...
movemouse(const Arg *arg)
{
	...
	Client *c = selmon->sel;
	if (!c || (c->isfullscreen && !c->isfakefullscreen) || c == selmon->overlay)
		return;
		...
		case MotionNotify:
			...
			if (ev.xmotion.y_root < selmon->my + bh + 100) {
				...
				|| ev.xmotion.y_root > selmon->my + selmon->mh) {
					Monitor *m = recttomon(ev.xmotion.x_root, ev.xmotion.y_root, 2, 2);
					if (m != selmon) {
						XRaiseWindow(dpy, c->win);
						sendmon(c, m);
						selmon = m;
	...
	Monitor *m = recttomon(c->x, c->y, c->w, c->h);
	if (!tagclient && m != selmon) {
		sendmon(c, m);
		selmon = m;
		focus(NULL);
	}
...
dragmouse(const Arg *arg)
{
	...
	if (!getrootptr(&x, &y))
		return;
	if (x > selmon->activeoffset + (1.0 / (double)selmon->bt) * selmon->btw - 30 
		...
		case MotionNotify:
			...
			if (!sinit) {
				...
				if (((abs((starty - ev.xmotion.y_root) *
					...
					|| (starty > 10 && ev.xmotion.y_root == 0 && c->isfloating))
				dragging = 1;
			}
		}
...
void drawwindow(const Arg *arg) {
	...
	(abs(c->w - width) > 20 || abs(c->h - height) > 20 || abs(c->x - x) > 20 || abs(c->y - y) > 20)) {
		Monitor *m = recttomon(x, y, width, height);
		if (m != selmon){
			sendmon(c, m);
			selmon = m;
			focus(NULL);
...
nexttiled(Client *c)
{
	while(c && (c->isfloating || !ISVISIBLE(c) || HIDDEN(c))){c = c->next;}
	return c;
}
...
resizeaspectmouse(const Arg *arg)
{
	...
	Client *c = selmon->sel;
	if (!c || (c->isfullscreen && !c->isfakefullscreen)) /* no support resizing fullscreen windows by mouse */
		return;
...
scan(void)
{
	...
	if (XQueryTree(dpy, root, &d1, &d2, &wins, &num)) {
		...
		for (size_t i = 0; i < num; ++i) { /* now the transients */
			if (!XGetWindowAttributes(dpy, wins[i], &wa))
				continue;
...
commandlayout(const Arg *arg) {
	int layoutnumber = (arg->ui || arg->ui >= LENGTH(layouts))
		? arg->ui : 0;
	Arg *larg = &((Arg) { .v = &layouts[layoutnumber] });
	setlayout(larg);
}
...
tagtoleft(const Arg *arg) {
	if (!selmon->sel){
		return;
	} else if (selmon->sel == selmon->overlay) {
        setoverlaymode(3);
        return;
    } else if (selmon->pertag->curtag == 1){
		return;
	}
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating) {
		...
		animateclient(selmon->sel, selmon->sel->sfx, selmon->sel->sfy, selmon->sel->sfw, selmon->sel->sfh, 7, 0);
	}
	else {
		XSetWindowBorder(dpy, selmon->sel->win, scheme[SchemeSel][ColBorder].pixel);
...
toggletag(const Arg *arg)
{
	...
	int ui = computeprefix(arg);
	unsigned int newtags = selmon->sel->tags ^ (ui & TAGMASK);
	if (newtags) {
...
void togglescratchpad(const Arg *arg) {
	...
	selmon->scratchvisible = !selmon->scratchvisible;
...
updatemotifhints(Client *c)
{
	...
	if (XGetWindowProperty(dpy, c->win, motifatom, 0L, 5L, False, motifatom,
		...
		unsigned long *motif = (unsigned long*)p;
...
updatesystray(void)
{
	...
	for (Client *c = systray->icons; c; c = c->next) {
		...
		w += systrayspacing;
		c->x = w;
		XMoveResizeWindow(dpy, c->win, c->x, 0, c->w, c->h);
		w += c->w;
		if (c->mon != m)
			c->mon = m;
	}
...
view(const Arg *arg)
{
	...
	if (ui & TAGMASK) {
        ...
        if (ui == ~0) {
            ...
            if ((pos + 1) == selmon->pertag->curtag)
				return;
			selmon->pertag->prevtag = selmon->pertag->curtag;
			selmon->pertag->curtag = pos + 1;
		}
	} else {
...
animleft(const Arg *arg) {
	...
	if (animated) {
		int tmpcounter = 0;
		for(Client *tempc = selmon->clients; tempc; tempc = tempc->next) {
...
viewtoleft(const Arg *arg) {
	...
	&& selmon->tagset[selmon->seltags] > 1) {
		...
		if (selmon->tagset[selmon->seltags ^ 1] >> 1 == ~0)
			...
			size_t i = 0;
			while (!(selmon->tagset[selmon->seltags ^ 1] >> 1 & 1 << i)) {++i;};
			selmon->pertag->curtag = i + 1;
		}
...
shiftview(const Arg *arg)
{
	...
	do {
		...
		for (Client *c = selmon->clients; c && !visible; c = c->next)
...
viewtoright(const Arg *arg) {
	...
	if(__builtin_popcount(selmon->tagset[selmon->seltags] & TAGMASK) == 1
		...
		if (selmon->tagset[selmon->seltags ^ 1] << 1 == ~0)
			...
			size_t i = 0;
			while (!(selmon->tagset[selmon->seltags ^ 1] << 1 & 1 << i)){
				++i;
			} 
			selmon->pertag->curtag = i + 1;
		}
...
void upscaleclient(const Arg *arg) {
	Client *c = !arg->v ? selmon->sel : (Client*)arg->v; 
	if(c)
		scaleclient(c, 30);
}
...
systraytomon(Monitor *m) {
	...
	size_t n = 1;
	for(Monitor *t = mons; t && t->next; ++n, t = t->next) ;
	if(systraypinningfailfirst && n < systraypinning)
		return mons;
	Monitor *t = mons;
	for(size_t i = 1; t && t->next && i < systraypinning; ++i, t = t->next) ;
	return t;
}
